...
function SkillModal({ close }: {close: () => void}) {
  ...
  useEffect(() => {
    addEventListener('mousedown', getTarget);
  ...
  return (
    <div className="fixed z-[100] flex justify-center items-center top-0 left-0 w-screen h-screen bg-black/20">
      <motion.div
        ...
        animate="animate"
        exit="initial"
        className="h-auto w-[350px] sm:w-[400px] lg:w-[450px] bg-zinc-200 dark:bg-[#2D3748] rounded-lg"
      >
        <div className="relative py-4 px-6">
          <div
            onClick={close}
            className="absolute right-6 hover:bg-slate-700 cursor-pointer rounded flex justify-center items-center p-2"
          >
            <IoMdClose />
          </div>
        ...
        <div
          id="scrollSKil"
          className="w-[100%] h-[400px] px-6 overflow-y-scroll"
        >
